...
>
> 2026-09-20 사용자 확인을 반영했다. 아래는 해당 범위의 보존 실험 결과이며, 유리한 ε만 선택하지 않고 전체를 보고한다. 기존 파일명·경로와 `provisional` 상태는 추적을 위해 유지한다. **보존 기록 분류: Provisional** — 현재 확정된 연구 범위와 구분하는 과거 실행 기록의 상태다. 별도 공식 실행계약의 승인·재실행 기록은 아직 연결되지 않았으며, 이 문서 수정으로 해당 기록을 생성하지 않는다.
