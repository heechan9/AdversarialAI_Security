"""Read-only independent audit of the archived experimental mean-filter defense run.
...
SOURCE_PATHS = frozenset({
    ...
    'src/adversarial_ai/defenses/mean.py',
    'src/adversarial_ai/evaluation/mean_defense_evaluation.py',
...
def audit(repo, root):
    ...
    with zipfile.ZipFile(io.BytesIO(archive)) as z:
        require(len(z.infolist()) == 12 and sum(i.file_size for i in z.infolist()) <= 2_000_000, 'ZIP size/inventory')
    ...
    expected_members = {'contract.json','summary.json','SHA256.json','COMPLETE.json'} | {
        f'{model}_eps_{epsilon:g}_samples.csv' for model in ('cnn','mobilenet')
        for epsilon in [float(r['epsilon']) for r in rows(
            (repo/f'results/attacks/provisional/fgsm_{model}.csv').read_bytes())]}
    require(set(members) == expected_members, 'member inventory')
    ...
    require(contract.get('defense_id') == 'mean' and contract.get('defense') ==
            'fixed 3x3 arithmetic mean /9, REFLECT, NHWC', 'defense contract')
    expected_inputs = {'configs/test_manifest.json','configs/classes.json',
        'results/clean/cnn_baseline_eval.csv','results/clean/mobilenet_eval.csv',
        'models/cnn_baseline.h5','models/mobilenet_finetuned.h5'}
    require(set(contract['inputs_sha256']) == expected_inputs, 'input inventory')
...
if __name__=='__main__':
    ...
    try:
        print(json.dumps(audit(args.repo,args.evidence or args.repo/'results/defenses/experimental/mean_run_01'),indent=2))
    except (ValueError,KeyError,OSError,TypeError,zipfile.BadZipFile) as exc:
        parser.exit(1,f'MEAN EVIDENCE AUDIT FAILED: {exc}\n')
