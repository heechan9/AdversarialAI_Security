...
def load_classes(classes_json_path: Path) -> list[str]:
    ...
    if len(set(indexed.values())) != len(indexed):
        raise VerificationError(f"Class labels must be unique: {classes_json_path}")
...
def read_clean_eval_csv(eval_csv_path: Path, classes: Sequence[str]) -> CleanEvalTable:
    ...
    if set(fieldnames) != set(REQUIRED_EVAL_COLUMNS) | {"correct"} | set(probability_columns):
        raise VerificationError(f"Clean evaluation CSV schema mismatch: {eval_csv_path}")
...
def recompute_confusion_matrix(
    ...
    if len(true_labels) != len(predicted_labels):
        raise VerificationError("True and predicted labels must have identical lengths")
    if not classes or len(set(classes)) != len(classes):
        raise VerificationError("Class labels must be nonempty and unique")
...
def check_report_csv(report_csv_path: Path, recomputed: dict[str, Any]) -> list[Check]:
    ...
    if fieldnames != ["", *REPORT_METRIC_KEYS]:
        raise VerificationError(f"Clean report CSV schema mismatch: {report_csv_path}")
...
def load_declared_models(experiment_yaml_path: Path) -> list[str]:
    ...
    if sum(line.split("#", 1)[0].rstrip() == "models:" for line in lines) != 1:
        raise VerificationError("Expected exactly one top-level models: block")
    ...
    for raw_line in lines:
        ...
        if indent == child_indent:
            ...
            if not re.fullmatch(r"[a-zA-Z0-9][a-zA-Z0-9_-]*", name):
                raise VerificationError(f"Unsafe model name in {experiment_yaml_path}: {name!r}")
            if name in names:
...
def _read_csv_rows(path: Path) -> tuple[list[dict[str, str]], list[str]]:
    ...
    if len(set(fieldnames)) != len(fieldnames):
        raise VerificationError(f"Duplicate CSV header: {path}")
    for row_number, row in enumerate(rows):
...
def _read_git_commit(repo_root: Path) -> str | None:
    """Resolve normal checkouts and worktrees; None explicitly means unavailable."""
...
def _write_report(output: Path, repo_root: Path, report: dict[str, Any]) -> None:
    """New files only; protect inputs, existing files, links and repository code."""
    output = output.absolute()
    for component in (output, *output.parents):
        if component.is_symlink() or getattr(component, "is_junction", lambda: False)():
            raise VerificationError(f"Unsafe report path: {output}")
    resolved = output.resolve()
    if resolved.is_relative_to(repo_root) and not resolved.is_relative_to(
        repo_root / "results" / "verification"
    ):
        raise VerificationError("Reports inside the checkout must be under results/verification")
    if resolved.suffix != ".json":
        raise VerificationError("Report output must be a new .json file")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional new JSON report; default prints only, existing files are never overwritten",
    )
    ...
    if args.output is not None:
        try:
            _write_report(args.output, repo_root, report)
        except (VerificationError, OSError, ValueError) as exc:
            print(f"Report output rejected: {exc}", file=sys.stderr)
            return 1
