...
import pandas as pd
import pytest
from adversarial_ai.evaluation import mean_defense_evaluation as ev
...
@pytest.mark.parametrize('defense', ['mean'])
def test_full_orchestration_synthetic_assets(tmp_path,monkeypatch,defense):
    ...
    out=tmp_path/'out'; ev.run(out, defense)
    assert len(calls)==2
    assert json.loads((out/'COMPLETE.json').read_text())['promoted'] is False
    contract=json.loads((out/'contract.json').read_text())
    assert contract['defense_id']==defense
    assert f'src/adversarial_ai/defenses/{defense}.py' in contract['source_files']
