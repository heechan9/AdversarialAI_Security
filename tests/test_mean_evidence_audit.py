...
spec=importlib.util.spec_from_file_location('independent_mean',REPO/'scripts/audit_mean_defense.py')
module=importlib.util.module_from_spec(spec);spec.loader.exec_module(module)


def test_archived_run():
    assert module.audit(REPO,REPO/'results/defenses/experimental/mean_run_01')['status']=='PASSED'


@pytest.fixture
def evidence(tmp_path):
    return Path(shutil.copytree(REPO/'results/defenses/experimental/mean_run_01',tmp_path/'evidence'))
...
def repack(root):
    ...
    with zipfile.ZipFile(b,'w') as z:
        for name in list(checks)+['SHA256.json','COMPLETE.json']:z.writestr(name,(root/name).read_bytes())
...
@pytest.mark.parametrize('change', ['defense', 'input_inventory', 'path'])
def test_mean_contract_and_path_mutations(evidence, change):
    if change == 'path':
        p=evidence/'cnn_eps_0_samples.csv'
        lines=p.read_text().splitlines();fields=lines[1].split(',');fields[0]='../escape.jpeg';lines[1]=','.join(fields);p.write_text('\n'.join(lines)+'\n')
    else:
        p=evidence/'contract.json';c=json.loads(p.read_text())
        if change == 'defense': c['defense_id']='gaussian'
        else: c['inputs_sha256']={}
