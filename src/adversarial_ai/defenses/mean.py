"""Fixed, differentiable 3x3 arithmetic mean comparison baseline.
...
def mean_numpy(images):
    ...
    for i in range(3):
        for j in range(3):
            out += (KERNEL[i][j] / 9.) * padded[:, i:i+x.shape[1], j:j+x.shape[2], :]
    return out


def mean_tensorflow(images):
    ...
    kernel = tf.reshape(tf.constant(KERNEL, tf.float32) / 9., (3, 3, 1, 1))
...
class MeanDefendedModel:
    ...
    def __call__(self, images, training=False):
        ...
        return self.model(mean_tensorflow(images), training=False)
...
def generate_adaptive_fgsm(model, images, labels, epsilon):
    ...
    attacked = generate_fgsm(MeanDefendedModel(model), images, labels, epsilon,
