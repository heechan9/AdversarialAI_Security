...
def _expected_root_files() -> set[str]:
    names = {"official_execution_context.json"}
...
def audit_official_candidate(
    ...
    candidate_path = candidate_dir or root / "results" / "attacks" / "official_candidate"
    expected_path = root / "results" / "attacks" / "official_candidate"
    if candidate_path.is_symlink() or not candidate_path.is_dir():
        raise AuditError(f"Official candidate directory is missing or unsafe: {candidate_path}")
    candidate = candidate_path.resolve()
    expected_candidate = expected_path.resolve()
    if candidate != expected_candidate:
        raise AuditError(f"Official candidate directory is missing or outside the fixed path: {candidate}")
    ...
    if context["output_dir"] != "results/attacks/official_candidate":
        raise AuditError("Official execution context output_dir is not the isolated candidate path")
    if context["epsilons"] != EXPECTED_EPSILONS:
    ...
    for model, spec in MODEL_SPECS.items():
        ...
        if metadata["epsilons"] != EXPECTED_EPSILONS:
    ...
    audit_models(manifest["manifest_models"], metadata_paths, repo_root=root)
    return {
        "status": "PASSED",
