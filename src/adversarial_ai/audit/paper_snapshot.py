...
def audit_paper_snapshot(
    ...
    for paper in classwise_rows:
        ...
        sample_path = (
            ...
            / "attacks"
            / "provisional"
            / f"fgsm_{model}_eps_{epsilon:g}_samples.csv"
        )
        frame = pd.read_csv(sample_path)
        class_rows = frame[(frame["true_label"] == class_name) & frame["clean_correct"].astype(bool)]
        _require_equal(paper["denominator"], len(class_rows), f"{model} {class_name} denominator")
        _require_equal(
            paper["successes"], int(class_rows["attack_success"].astype(bool).sum()),
    ...
    review_frame = pd.read_csv(
        repo_root / "results" / "audit" / "evidence" / "63_images_strict_visual_audit.csv",
        encoding="utf-8-sig",
    )
    paper_visual = claims["visual_review"]
    _require_equal(paper_visual["total"], visual["total_candidates"], "visual total")
    _require_equal(
        paper_visual["split_counts"], dict(Counter(review_frame["검토자"])), "visual split counts"
    )
    _require_equal(
        paper_visual["judgment_counts"], dict(Counter(review_frame["판정"])),
