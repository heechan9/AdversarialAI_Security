...
export default function Ocean({playing,command,onInteract,onZoom}:{playing:boolean,command:{id:number;kind:ShipCommand},onInteract:()=>void,onZoom:(zoom:number)=>void}){
 ...
 useEffect(()=>{
  ...
  const geo=new THREE.BufferGeometry();geo.setAttribute("position",new THREE.Float32BufferAttribute(verts,3));geo.setIndex(idx);geo.computeVertexNormals();hullMat.side=THREE.DoubleSide;const hull=new THREE.Mesh(geo,hullMat);hull.castShadow=true;hull.receiveShadow=true;ship.add(hull);
  ...
  ship.add(new THREE.LineSegments(new THREE.BufferGeometry().setFromPoints(rails),railMaterial));
  function cylinder(radius:number,height:number,x:number,y:number,z:number,m:THREE.Material){const mesh=new THREE.Mesh(new THREE.CylinderGeometry(radius,radius,height,12),m);mesh.position.set(x,y,z);mesh.castShadow=true;ship.add(mesh);return mesh;}
  ...
  for(let x=-11;x<8;x+=1.35){const port=cylinder(.11,.025,x,1.05,-2.13,darkMat);port.rotation.x=Math.PI/2;}
