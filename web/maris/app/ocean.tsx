...
import {createShipControls,resetShipControls} from "@/lib/ship-controls";

export default function Ocean({playing,command,onInteract,onZoom}:{playing:boolean,command:{id:number;kind:"reset"|"in"|"out"},onInteract:()=>void,onZoom:(zoom:number)=>void}){
 ...
 useEffect(()=>{
  ...
  scene.add(new THREE.HemisphereLight(0xc3dfeb,0x172b38,2));const sun=new THREE.DirectionalLight(0xffedcf,3);sun.position.set(15,30,-18);scene.add(sun);const rim=new THREE.DirectionalLight(0x84c6df,1.8);rim.position.set(-22,12,20);scene.add(rim);
  ...
  const hullMat=mat(0x496371,.32,.4),deckMat=mat(0x394e59,.12,.87),lightMat=mat(0x9fadb0,.25,.48),darkMat=mat(0x102735,.45,.24),lineMat=mat(0xcac7b3,.05,.8),accentMat=mat(0x92b3bd,.5,.32);
  function box(w:number,h:number,d:number,x:number,y:number,z:number,m:THREE.Material){const o=new THREE.Mesh(new THREE.BoxGeometry(w,h,d),m);o.position.set(x,y,z);o.castShadow=h>.3;o.receiveShadow=true;ship.add(o);return o}
  ...
  const geo=new THREE.BufferGeometry();geo.setAttribute("position",new THREE.Float32BufferAttribute(verts,3));geo.setIndex(idx);geo.computeVertexNormals();hullMat.side=THREE.DoubleSide;ship.add(new THREE.Mesh(geo,hullMat));
  const shape=new THREE.Shape();outline.forEach(([x,z],i)=>i?shape.lineTo(x,-z):shape.moveTo(x,-z));shape.closePath();const deck=new THREE.Mesh(new THREE.ExtrudeGeometry(shape,{depth:.18,bevelEnabled:true,bevelThickness:.025,bevelSize:.025,bevelSegments:1}),deckMat);deck.rotation.x=-Math.PI/2;deck.position.y=1.44;deck.receiveShadow=true;ship.add(deck);
  ...
  const detailMat=new THREE.LineBasicMaterial({color:0x8aa4ad,transparent:true,opacity:.28});materials.push(detailMat);ship.add(new THREE.LineSegments(new THREE.BufferGeometry().setFromPoints(detailPoints),detailMat));
  ...
  ship.add(new THREE.LineSegments(new THREE.BufferGeometry().setFromPoints(seamPoints),detailMat));
  ...
  for(let x=-11;x<12;x+=1.1){box(.028,.25,.028,x,1.65,2.03,accentMat);box(.028,.25,.028,x,1.65,-2.03,accentMat)}
  box(23,.022,.022,.1,1.78,2.03,accentMat);box(23,.022,.022,.1,1.78,-2.03,accentMat);
  ...
  const radar=new THREE.Mesh(new THREE.SphereGeometry(.32,16,12),lightMat);radar.position.set(-3.6,7.13,1.28);ship.add(radar);
