...
## ACK 2026 v2.5 기반 원고 편집 기여 (2026-09-16 이력)

- **김기호**: FGSM 검토 기록과 논문 초안 제공. ASR 정의, 기록 대조 및 비단조 결과 해석에 반영. 모델 추론의 독립 재실행 기여로 기재하지 않음.
- **김태희**: 담당 31장 직접 확인 회신, 관찰 특징 및 Claude의 용어·판단 기준 보조 사용 범위 공개.
- **이재혁**: 담당 32장 직접 확인 회신과 DDG 판단 이유 보완. 일반 군함 관찰과 DDG 확정의 차이는 미해결 항목으로 유지.
- **최희찬**: 실험 결과 제공, 팀 검토 조율, 원고 통합 및 GitHub 게시 요청.
- **Codex**: 최신 근거 대조, 동적 표 생성, 원고 편집·조판 및 검토 경계 문서 작성.

이 기록은 논문 저자 순서·공동 1저자·정식 결과 채택을 확정하지 않습니다. 새 회신을 기존 canonical 라벨이나 고정 검토 CSV로 덮어쓰지 않습니다.

위 역할 기록의 후속 해석: 김태희의 비고 설명 확인을 선종 판정과 합산하지 않는다. 최신 원고 검증·게시 상태는 docs/PAPER_RELEASE_STATUS.md를 따른다.
